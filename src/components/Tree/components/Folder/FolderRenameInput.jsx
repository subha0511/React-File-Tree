...
import { resetRename, setNewName } from "./FolderState";
...
function FolderRenameInput({ path, state, dispatch }) {
  const { state: treeState, dispatch: treeDispatch } = useTreeContext();

  useEffect(() => {
    const handleGlobalKeydown = onEnterKeyPress(handleFolderRename);
